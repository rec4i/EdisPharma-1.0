    ...
    public partial class hizmetler : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (EczaneID != null)
            {
                ...
                RepeaterSiparisBilgi.DataBind();
                RepeaterSiparişOnayModal.DataSource = dt3;

                RepeaterSiparişOnayModal.DataBind();
                SqlC.con.Close();
            ...
            SqlC.con.Open();
            SqlCommand cmd1 = new SqlCommand(" use kasa select *,Eczane_ADI from Siparisler3 ,ECZANE2 where Siparisler3.OnayDurum=0 and ECZANE2.EczaneID=Siparisler3.EczaneID;");
        ...
        public void EczaneStoguna_Ekle()
        {
            if (tipi != "0")
            {
                ...
                //StokUrunadı ÇEKME
                SqlC.con.Open();
                SqlC.com.CommandText = " use kasa select UrunAdıID as UrunAdıSayısı  from siparis where EczaneID='" + EczaneID + "' and SiparislerID='" + siparislerIDsi + "'";
                SqlDataReader da1 = SqlC.com.ExecuteReader();
                for (int i = 0; i < StokUrunadet; i++)
                {
                    if (da1.Read())
                    {
                        StokUrunadları[i] = Convert.ToString(da1[0]);
                    }
                    else
                    {
                        break;
                ...
                for (int i = 0; i < Eczane_StokUrunAdet; i++)
                {
                    ...
                    {
                        if (Eczane_StokUrunadları[i].Trim() == StokUrunadları[j])
                        {
                            SqlC.con.Open();
