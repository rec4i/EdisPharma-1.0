<%@ Page Title="" Language="C#" MasterPageFile="~/Bs.Master" AutoEventWireup="true" CodeBehind="Yıllık-İzin-Onay.aspx.cs" Inherits="deneme9.Yıllık_İzin_Onay" %>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            function Tabloyu_Doldur() {
                ...
                $.ajax({
                    url: 'Yıllık-İzin-Onay.aspx/Tablo_Doldur',
                    type: 'POST',
                    ...
                    success: function (data) {
                        ...
                        for (var i = 0; i < parsdata.length; i++) {
                            ...
                            Tbody.append('<tr>' +
                                ...
                                '<td>' + Onay_Label + '</td>' +
                                '<td>' + Onay_Label + '</td>' +
                                '<td><a style="font-size: 20px; " id="Numune_Talebi_Kaldır" value="' + parsdata[i].İzin_Id + '"><i class="fa fa fa-search"></i></a></td>' +
                ...
                Numune_Talebi_Kaldır.click(function () {
                    $('#myModal').modal('show');
                    Onay_Talebi_Gonder_Modal.attr("value", $(this).attr('value'))
                    Red_Talebi_Gonder_Modal.attr("value", $(this).attr('value'))
                })
            ...
            Tsm_Ad.change(function () {
                İzinleri_Doldur();
            });
            function İzinleri_Doldur() {
                var Tsm_Ad_Kullanıcı_Id = Tsm_Ad.find('option:selected').attr('value')
                $.ajax({
                    url: 'Yıllık-İzin-Onay.aspx/Kullanıcı_Bilgileri',
                    dataType: 'json',
                    type: 'POST',
                    async: false,
                    data: "{'Kullanıcı_Id': '" + Tsm_Ad_Kullanıcı_Id+"'}",
                    contentType: 'application/json; charset=utf-8',
                    success: function (data) {
            ...
            Onay_Talebi_Gonder_Modal.click(function () {
                $.ajax({
                    url: 'Yıllık-İzin-Onay.aspx/Talebi_Onayla',
                    dataType: 'json',
                    type: 'POST',
                    async: false,
                    data: "{'İzin_Id': '" + $(this).attr('value') + "','Onay_Durum':'1'}",
                    contentType: 'application/json; charset=utf-8',
                    success: function (data) {
                        Tabloyu_Doldur();
                        ...
                        if (data.d == "0") {
                            $('#Mesaj').find('p[id=İşlem_Mesajı]').html("Talep Başarı İle Onaylanmıştır")
                        }
                        if (data.d == "2") {
                            $('#Mesaj').find('p[id=İşlem_Mesajı]').html("İşlem Sırasında Bir Hata Oluştu Lütfen Gerekli Yerlere Bildiriniz")
                        }
            ...
            var Red_Talebi_Gonder_Modal = $('button[id*=Red_Talebi_Gonder_Modal]')
            Red_Talebi_Gonder_Modal.click(function () {
                $.ajax({
                    url: 'Yıllık-İzin-Onay.aspx/Talebi_Onayla',
                    dataType: 'json',
                    type: 'POST',
                    async: false,
                    data: "{'İzin_Id': '" + $(this).attr('value') + "','Onay_Durum':'2'}",
                    contentType: 'application/json; charset=utf-8',
                    success: function (data) {
                        Tabloyu_Doldur();
                        $('#myModal').modal('toggle');
        ...
        <div class="vertical-alignment-helper">
            <div class="modal-dialog vertical-align-center">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Plan Onaylandıktan Sonra Kullanıcı Düzenleme  Yapamaz !

                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Kapat</button>
                        <button type="button" id="Red_Talebi_Gonder_Modal" class="btn btn-danger">Talebi Reddet</button>
                        <button type="button" id="Onay_Talebi_Gonder_Modal" class="btn btn-primary">Talebi Onayla</button>
