<%@ Page Title="" Language="C#" MasterPageFile="~/b.Master" AutoEventWireup="true" CodeBehind="B-Tsm-Plan-Raporlama.aspx.cs" Inherits="deneme9.B_Tsm_Plan_Raporlama" %>
        ...
        $(document).ready(function () {
            ...
            cal_set.on('click', function () {
                if (Tsm_Ad.find('option:selected').val() != 0) {
                    window.location.href = "B-Tsm-Plan-Raporlama.aspx?x=" + TextBox2.val() + "&y=" + TextBox3.val() + "&z=" + Tsm_Ad.find('option:selected').val()
                }
                else {
            ...
            $.ajax({
                url: 'B-Tsm-Plan-Raporlama.aspx/Tabloları_Doldur',
                dataType: 'json',
                type: 'POST',
                data: "{'parametre': '" + TextBox2.val() + "*" + TextBox3.val() + "*" + Kullanıcı + "'}",
                contentType: 'application/json; charset=utf-8',
                success: function (data) {
                    ...
                    for (var i = 0; i < data.d.split('!').length; i++) {// '<td><span class="label label-waring">Ziyret bekleniyor</span></td>'
                        ...
                        var Doktor_Div = $('tbody[id = Gun_Doktor_' + parseInt(data.d.split('!')[i].split('/')[6]) + ']');
                        Doktor_Div.append('<tr><td>' + data.d.split('!')[i].split('/')[0] + '</td><td>' + data.d.split('!')[i].split('/')[2] + '</td><td>' + data.d.split('!')[i].split('/')[3] + '</td><td>' + data.d.split('!')[i].split('/')[4] + '</td></tr>')
                    }
                },
                error: function () {
            ...
            $.ajax({
                url: 'B-Tsm-Plan-Raporlama.aspx/Tabloları_Doldur_Eczane',
                dataType: 'json',
                type: 'POST',
                data: "{'parametre': '" + TextBox2.val() + "*" + TextBox3.val() + "*" + Kullanıcı + "'}",
                contentType: 'application/json; charset=utf-8',
                success: function (data) {
                    ...
                    for (var i = 0; i < data.d.split('!').length; i++) {// '<td><span class="label label-waring">Ziyret bekleniyor</span></td>'
                        ...
                        var Doktor_Div = $('tbody[id = Gun_Eczane_' + parseInt(data.d.split('!')[i].split('/')[5]) + ']');
                        Doktor_Div.append('<tr><td>' + data.d.split('!')[i].split('/')[0] + '</td><td>' + data.d.split('!')[i].split('/')[3] + '</td><td>' + data.d.split('!')[i].split('/')[2] + '</td></tr>')
            ...
            Hepsini_Yazdır.click(function () {
                ...
                for (var i = 0; i < veri.length; i++) {
                    ...
                    if (Doktor_Sayısı == 0) {
                        Doktor_Tablo.push(
                            [
                                { text: 'Tabloda Hiç Veri Bulunamadı', colSpan: 4, alignment: 'center' }
                            ]
                        );
                    }
                    if (Eczane_Sayısı == 0) {
                        Eczane_Tablo.push(
                            [
                                { text: 'Tabloda Hiç Veri Bulunamadı', colSpan: 3, alignment: 'center' }
                            ]
                        );
                    }
