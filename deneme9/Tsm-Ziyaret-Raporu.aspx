<%@ Page Title="" Language="C#" MasterPageFile="~/bs.Master" AutoEventWireup="true" CodeBehind="Tsm-Ziyaret-Raporu.aspx.cs" Inherits="deneme9.Tsm_Ziyaret_Raporu" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                url: 'Tsm-Ziyaret-Raporu.aspx/Kullanıcı_Listesi',
                type: 'POST',
                data: "{'Şehir_Id': ''}",
                async: false,
                dataType: "json",
            ...
            if (window.location.href.split('&').length > 1) {
                ...
                var Grafik_Div = $('div[id=Grafik_Div]')
                Grafik_Div.attr('style', "visibility:visible")
                ...
                Grafik_Div.append(myvar)

                //$.ajax({
                //    url: 'Tsm-Ziyaret-Raporu.aspx/Tabloları_Doldur_Eczane_Grafik',
                //    dataType: 'json',
                //    type: 'POST',
                //    async:false,
                ...
                //            label.push(temp[i].Urun_Adı)
                //        }
                //        for (var i = 0; i < temp.length; i++) {
                //            datas.push(temp[i].Adet)
                //        }
                //        console.log(data.d)
            ...
            cal_set.on('click', function () {
                if (Tsm_Ad.find('option:selected').val() != 0) {
                    window.location.href = "Tsm-Ziyaret-Raporu.aspx?x=" + TextBox2.val() + "&y=" + TextBox3.val() + "&z=" + Tsm_Ad.find('option:selected').val()
                }
                else {
            ...
            var parsdata;
            $.ajax({
                url: 'Tsm-Ziyaret-Raporu.aspx/Ziyaret_Edilecekler',
                dataType: 'json',
                type: 'POST',
                async: false,
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < temp.length; i++) {// '<td><span class="label label-waring">Ziyret bekleniyor</span></td>'
                        ...
                        var Çalışılan_Urunler = "";

                        for (var j = 0; j < temp[i].Çalışılan_Urunler.split(';').length; j++) {
                            Çalışılan_Urunler += '-' + temp[i].Çalışılan_Urunler.split(';')[j] + ' <br>'
                        }
                        ...
                        Doktor_Div.append('<tr><td>' + temp[i].Doktor_Ad + '</td><td>' + temp[i].Brans + '</td><td>' + temp[i].Unite + '</td><td>' + temp[i].Brick + '</td>' + label_str + '<td>' + Çalışılan_Urunler + '</td><td>' + temp[i].Ziyaret_Notu + '</td></tr>')
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < temp.length; i++) {// '<td><span class="label label-waring">Ziyret bekleniyor</span></td>'
                        ...
                        Doktor_Div.append('<tr><td>' + temp[i].Eczane_Adı + '</td><td>' + temp[i].Şehir + '</td><td>' + temp[i].Brick + '</td>' + label_str + '<td>' + Çalışılan_Urunler + '</td><td>' + temp[i].Ziyaret_Notu + '</td></tr>')
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <%--    <div id="Grafik_Div" style="visibility: hidden"></div>
