<%@ Page Title="" Language="C#" MasterPageFile="~/b.Master" AutoEventWireup="true" CodeBehind="B-Tsm-Masraf_Raporlama.aspx.cs" Inherits="deneme9.B_Tsm_Masraf_Raporlama" %>
        ...
        $(document).ready(function () {
            ...
            cal_set.on('click', function () {
                if (Tsm_Ad.find('option:selected').val() != 0) {
                    window.location.href = "B-Tsm-Masraf_Raporlama.aspx?x=" + TextBox2.val() + "&y=" + TextBox3.val() + "&z=" + Tsm_Ad.find('option:selected').val()
                }
                else {
            ...
            function Tabloyu_Doldur() {
                ...
                $.ajax({
                    url: 'Tsm-Masraf-Raporu.aspx/Tablo_Doldur',
                    type: 'POST',
                    ...
                    success: function (data) {
                        ...
                        for (var i = 0; i < parsdata.length; i++) {
                            Tbody.append('<tr><td>' + parsdata[i].Doktor_Ad + '</td><td>' + parsdata[i].Tarih + '</td><td>' + parsdata[i].Tur_Txt + '</td><td>' + parsdata[i].Belge_Tur_Txt + '</td><td>' + parsdata[i].Sirket_Unvanı + '</td><td style="word-break:break-all;" >' + parsdata[i].Açıklama + '</td><td>' + parsdata[i].Kdv_Oranı + '</td><td>' + parsdata[i].Kdv_Tutarı.replace(',', '.') + '</td><td>' + parsdata[i].Kdv_Haric_Tutar.replace(',', '.') + '</td><td>' + parsdata[i].Toplam_Tutar.replace(',', '.') + '</td></tr>')
                        }
                ...
                var Kullanıcı_Adı;
                $.ajax({
                    url: 'Tsm-Masraf-Raporu.aspx/Kullanıcı_Adı_Soyadı',
                    type: 'POST',
                    data: "{'parametre': '" + Kullanıcı+"'}",
                    async: false,
                    dataType: "json",
                    contentType: "application/json; charset=utf-8",
                    success: function (data) {
