<%@ Page Title="" Language="C#" MasterPageFile="~/admin.Master" AutoEventWireup="true" CodeBehind="Kota-Realizasyon-Takip.aspx.cs" Inherits="deneme9.Kota_Realizasyon_Takip" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var calmth_İncele = $('select[id=calmth]')

            var calyr_İncele = $('select[id=calyr')

            var Yıl = calyr_İncele.val()
            var Ay = calmth_İncele.val()
            $.ajax({
                url: 'Kota-Realizasyon-Takip.aspx/Kota_Detay',
                dataType: 'json',
                type: 'POST',
                async: false,
                ...
                if (Liste_.length > 0) {
                    ...
                    for (var i = 0; i < Liste_.length; i++) {
                        ...
                        Tbody.append(
                            ...
                            '<td>' + Liste_[i].Adet + '</td>' +
                            '<td>' + Liste_[i].Hedef.replace(',', '.').replace(',', '.') + '₺</td>' +
                            '<td>' + Liste_[i].Tamamlanan.replace(',', '.').replace(',', '.') + '₺</td>' +
                            '<td>' + parseInt(parseInt(Liste_[i].Hedef.replace(',', '.').replace(',', '.')) - parseInt(Liste_[i].Tamamlanan.replace(',', '.').replace(',', '.'))) + '₺</td>' +
                            '<td>%' + Liste_[i].Yüzde.replace(',', '.').replace(',', '.') + '</td>' +
                            '</tr>'
                        )
                    }
