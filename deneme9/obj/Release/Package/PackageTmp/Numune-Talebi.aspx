<%@ Page Title="" Language="C#" MasterPageFile="~/admin.Master" AutoEventWireup="true" CodeBehind="Numune-Talebi.aspx.cs" Inherits="deneme9.Numune_Talebi" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            function Tabloyu_Doldur() {
                ...
                $.ajax({
                    url: 'Numune-Talebi.aspx/Tablo_Doldur',
                    type: 'POST',
                    ...
                    success: function (data) {
                        ...
                        for (var i = 0; i < parsdata.length; i++) {
                            Tbody.append(
                                '<tr><td>' + parsdata[i].Doktor_Ad + '</td>' +
                                '<td>' + parsdata[i].Tarih + '</td>' +
                                '<td>' + parsdata[i].UrunADI + '</td>' +
                                '<td>' + parsdata[i].Adet + '</td>' +

                                '<td><a style="font-size: 20px; " id="Numune_Talebi_Kaldır" value="' + parsdata[i].Numune_Talebi_Id + '"><i class="fa fa-trash-o"></i></a></td>' +
                                '</tr>'
                            )
                        }
                ...
                var Kullanıcı_Adı;
                $.ajax({
                    url: 'Numune-Talebi.aspx/Kullanıcı_Adı_Soyadı',
                    type: 'POST',
                    data: "{'parametre': ''}",
                    async: false,
                    dataType: "json",
                    ...
                    buttons: [
                        {
                            extend: 'excelHtml5',
                            title: function () {
                                return "Numune_Talebi" + "_" + Kullanıcı_Adı + "_" + TextBox2.val() + "_" + TextBox3.val();
                            },
                        ...
                        },
                        {
                            ...
                            customize: function (doc) {
                                ...
                                //var temp = doc.content[1].table.body;
                                //console.log(data)
                                //doc.content[0] = [
                                //    {
                                //        margin: 20,
                ...
                Numune_Talebi_Kaldır.click(function () {
                    $.ajax({
                        url: 'Numune-Talebi.aspx/Numune_Talebi_Kaldır',
                        dataType: 'json',
                        type: 'POST',
            ...
            var Talep_Oluştur = $('button[id=Talep_Oluştur]')
            Talep_Oluştur.click(function () {
                var İlaç_Adı = $('select[id=İlaç_Adı]')
                ...
                İlaç_Adı.parent().removeAttr('class')//form-group has-error
                İlaç_Adı.parent().attr('class', 'form-group')


                Adet.parent().removeAttr('class')//form-group has-error
                ...
                var Doktor_Listesi_ = true, Şehir_ = true, Brick_ = true,
                    Unite_ = true, Branş_ = true, Doktor_ = true, İlaç_Adı_ = true
                Adet_ = true
                ...
                if (İlaç_Adı.find('option:selected').attr('value') == "" || İlaç_Adı.find('option:selected').attr('value') == "undefined" || İlaç_Adı.find('option:selected').attr('value') == undefined || İlaç_Adı.find('option:selected').html() == "--&gt;&gt; Lütfen İlaç Seçiniz &lt;&lt;--") {
                    İlaç_Adı.parent().removeAttr('class')//form-group has-error
                    İlaç_Adı.parent().attr('class', 'form-group has-error')
                    İlaç_Adı_ = false;
                }

                if (Adet.val() == "" || Adet.val() == "undefined" || Adet.val() == undefined) {
                    Adet.parent().removeAttr('class')//form-group has-error
                ...
                if (Doktor_Listesi_ == true && Şehir_ == true && Brick_ == true && Unite_ == true && Branş_ == true && Doktor_ == true && İlaç_Adı_ == true && Adet_ == true) {
                    console.log("assda")
                    $.ajax({
                        url: 'Numune-Talebi.aspx/Talep_Oluştur',
                        dataType: 'json',
                        type: 'POST',

                        data: "{'Doktor_Id': '" + Doktor_Adı_Listesi.find('option:selected').attr('value') + "'," +
                            "'İlaç_Adı': '" + İlaç_Adı.find('option:selected').attr('value') + "'," +
                        ...
                        success: function (data) {
                            ...
                            $.ajax({
                                url: 'Numune-Talebi.aspx/OrnekPost',
                                dataType: 'json',
                                type: 'POST',
                                data: "{'parametre': '0-0'}",
                                contentType: 'application/json; charset=utf-8',
                            ...
                            $.ajax({
                                url: 'Numune-Talebi.aspx/İlaç_Listesi',
                                dataType: 'json',
                                type: 'POST',
                                data: "{'parametre': '0-0'}",
                                contentType: 'application/json; charset=utf-8',
                                success: function (data) {
                                    İlaç_Adı.empty();
                                    İlaç_Adı.append("<option>-->> Lütfen İlaç Seçiniz <<--</option>");
            ...
            $.ajax({
                url: 'Numune-Talebi.aspx/Yeni_Liste_Olustur_Listeler',
                dataType: 'json',
                type: 'POST',
                data: "{'parametre': '0-0'}",
                contentType: 'application/json; charset=utf-8',
    ...
    <div class="box">
        <div class="box-body">
            ...
            <div class="row">
                <div class="col-xs-6">
                    <div class="form-group">
                        ...
                        <select id="İlaç_Adı" class="form-control">
                        </select>
                    </div>
                </div>
                <div class="col-xs-6">
                    <div class="form-group">
                        <label>Adet</label>
                        <input id="Adet" type="text" class="form-control" placeholder="100" />
            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="form-group">
                        <button id="Talep_Oluştur" type="button" class="btn btn-block btn-info btn-lg">Talep Oluştur</button>
                    </div>
                </div>
            </div>
            <div class="row">
