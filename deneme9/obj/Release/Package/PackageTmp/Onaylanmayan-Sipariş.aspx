<%@ Page Title="" Language="C#" MasterPageFile="~/Bs.Master" AutoEventWireup="true" CodeBehind="Onaylanmayan-Sipariş.aspx.cs" Inherits="deneme9.Onaylanmayan_Sipariş" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $.ajax({
                url: 'Onaylanmayan-Sipariş.aspx/Eczane_Sipariş_İptal_Genel_Sorgu',
                type: 'POST',
                data: "{'Sipariş_Id': ''}",
                async: false,
                dataType: "json",
                ...
                success: function (data) {
                    ...
                    if (temp.length > 0) {
                        for (var i = 0; i < temp.length; i++) {
                            ...
                            MyClass.Görüsme_Sonucu = temp[i].Görüsme_Sonucu
                            MyClass.Ad_Soyad = temp[i].Ad_Soyad
                            MyClass.Bm_Ye_Gönderildimi = temp[i].Bm_Ye_Gönderildimi
            ...
            function Tabloyu_Doldur(Liste_) {
                $('#Eczna_Sipariş_İptal_Tablo_Div').empty();

                $('#Eczna_Sipariş_İptal_Tablo_Div').append('<table id="example" class="display" style="width: 100%">' +
                    '<thead>' +
                    '<tr>' +
                    '<th>TSM</th>' +
                    '<th>TSM Cepavladımı</th>' +
                    '<th>Eczane Adı</th>' +
                    '<th>Onay Durumu</th>' +
                    '<th>İletim Durumu</th>' +
                ...
                if (Liste_.length > 0) {
                    ...
                    for (var i = 0; i < Liste_.length; i++) {
                        ...
                        var Tsm_Cevapladımı_Label = '';
                        if (Liste_[i].Bm_Ye_Gönderildimi == 0) {
                            Tsm_Cevapladımı_Label = '<span class="label label-warning">HAYIR</span>'

                        }
                        if (Liste_[i].Bm_Ye_Gönderildimi == 1) {
                            Tsm_Cevapladımı_Label = '<span class="label label-info">EVET</span>'
                        ...
                        Tbody.append(
                            '<tr>' + 
                            '<td>' + Liste_[i].Ad_Soyad + '</td>' +
                            '<td>' + Tsm_Cevapladımı_Label + '</td>' +
                            ...
                            '<td>' + Liste_[i].Eposta + '</td>' +

                            '<td>' + '<a value="' + Liste_[i].Detay + '" Görüsme_Sonucu="' + Liste_[i].Görüsme_Sonucu+'" id="Sipariş_Detay"><i class="fa fa fa-search"></i></a>' + '</td>' +
                            '</tr>'
                        )
                    }
            ...
            //#region  Detay
            var Siparişi_İptal_Et = $('button[id=Siparişi_İptal_Et]')
            var Siparişi_Tekrar_Oluştur = $('button[id=Siparişi_Tekrar_Oluştur]')
            var Tekrar_Sipariş_Görüşmesi_İste = $('button[id=Tekrar_Sipariş_Görüşmesi_İste]')

            var Sipariş_Detay = $('a[id=Sipariş_Detay]')
            Sipariş_Detay.click(function () {
                Siparişi_İptal_Et.attr('sipariş_Id', $(this).attr("value"))
                Siparişi_Tekrar_Oluştur.attr('sipariş_Id', $(this).attr("value"))
                Tekrar_Sipariş_Görüşmesi_İste.attr('sipariş_Id', $(this).attr("value"))

             
                $('textarea[id=Eczane_Ekle_Cıkar_Duzenle_Adres]').val($(this).attr('görüsme_sonucu'))
            ...
            $('#Eczane_Eksik_Bilgi').on('shown.bs.modal', function () {
                setTimeout(function () { $('#example').DataTable().rows().invalidate().draw() }, 450);
            });
            $('#Ziyaret_Modal').on('shown.bs.modal', function () {
            ...
            Siparişi_İptal_Et.click(function () {
           
                $.ajax({
                    url: 'Onaylanmayan-Sipariş.aspx/Siparişi_İptal_Et',
                    type: 'POST',
                    data: "{'sipariş_Id': '" + $(this).attr('sipariş_Id')+"'}",
                    async: false,
                    dataType: "json",
                    contentType: "application/json; charset=utf-8",
                    success: function (data) {
            ...
            Siparişi_Tekrar_Oluştur.click(function () {
            
                $.ajax({
                    url: 'Onaylanmayan-Sipariş.aspx/Siparişi_Tekrar_Oluştur',
                    type: 'POST',
                    data: "{'sipariş_Id': '" + $(this).attr('sipariş_Id') + "'}",
                    async: false,
                    dataType: "json",
            ...
            Tekrar_Sipariş_Görüşmesi_İste.click(function () {
          
                $.ajax({
                    url: 'Onaylanmayan-Sipariş.aspx/Tekrar_Sipariş_Görüşmesi_İste',
                    type: 'POST',
                    data: "{'sipariş_Id': '" + $(this).attr('sipariş_Id') + "'}",
                    async: false,
                    dataType: "json",
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div id="Ziyaret_Modal" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="Tekrar_Sipariş_Görüşmesi_İste" class="btn btn-warning" data-dismiss="modal">Tsm'den Tekrar Sipariş Görüşmesi İste</button>
                    <button type="button" id="Siparişi_Tekrar_Oluştur" class="btn btn-success" data-dismiss="modal">Siparişi Tekrar Oluştur</button>
                    <button type="button" id="Siparişi_İptal_Et" class="btn btn-danger" data-dismiss="modal">Siparişi İptal Et</button>
        ...
        <div class="box">
            <div class="box-body">
                <div id="Eczna_Sipariş_İptal_Tablo_Div"></div>
